...
const buildHistoryFromLogs = (
  ...
  const recentLogs = logsNewestFirst
    .filter((log): log is BackendMonitorLog => log.status === 'up' || log.status === 'down')
    .slice(0, barCount);
  ...
  // Keep the oldest visible state as the fill value so missing buckets do not
  // invent artificial transitions when we only have a partial log window.
  const oldestKnownState: HistoryBarState =
    recentLogs[recentLogs.length - 1]?.status === 'down' ? 'down' : 'up';
  const history = Array.from({ length: barCount }, () => oldestKnownState);
  const orderedLogsOldestFirst = [...recentLogs].reverse();
  const startIndex = barCount - orderedLogsOldestFirst.length;

  orderedLogsOldestFirst.forEach((log, index) => {
    history[startIndex + index] = log.status;
  });
...
export const buildMonitorHistoryBars = ({
  ...
  if (logsNewestFirst.length > 0) {
    return buildHistoryFromLogs(logsNewestFirst, status, uptime, barCount);
  }
