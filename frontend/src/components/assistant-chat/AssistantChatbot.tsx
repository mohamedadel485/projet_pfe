...
const DEFAULT_SYSTEM_PROMPT =
  "You are a helpful, clear, and friendly assistant. Reply in English unless the user asks for another language.";
const EMOJI_OPTIONS = [
  "😀",
  "😄",
  "😊",
...
const createConversationFileName = (date = new Date()): string =>
  `conversation-uptime-warden-${date
    .toISOString()
    .replace(/\.\d{3}Z$/, "Z")
    .replace(/:/g, "-")}.txt`;
...
const URL_CANDIDATE_PATTERN =
  /((?:(?:https?|wss?)?:\/\/)?(?:www\.)?[a-z0-9-]+(?:\.[a-z0-9-]+)+(?:\/[^\s]*)?)/i;
const URL_ONLY_PATTERN =
  /^\s*((?:(?:https?|wss?)?:\/\/)?(?:www\.)?[a-z0-9-]+(?:\.[a-z0-9-]+)+(?:\/[^\s]*)?)\s*$/i;
...
const parseDurationToSeconds = (
  value: string,
  defaultUnit: "seconds" | "minutes",
): number | undefined => {
  const normalized = normalizeMonitorToken(value).replace(/\s+/g, "");
  const match = normalized.match(
    /^(\d+)(s|sec|secs|second|seconds|m|min|mins|minute|minutes|h|hr|hrs|hour|hours)?$/i,
  );
  if (!match) {
    return undefined;
  }
  ...
  const unit =
    match[2]?.toLowerCase() ?? (defaultUnit === "minutes" ? "m" : "s");
...
const parseIpVersion = (value: string): MonitorIpVersion | undefined => {
  ...
  if (normalized.includes("ipv6") && normalized.includes("ipv4")) {
    return normalized.startsWith("ipv6")
...
const parseUpStatusCodeGroups = (
  value: string,
): MonitorUpStatusCodeGroup[] | undefined => {
  const normalized = normalizeMonitorToken(value);
...
const parseStructuredMonitorFields = (
  rawText: string,
): StructuredMonitorFields => {
  ...
  if (
    (fields.authUsername || fields.authPassword) &&
    fields.authType === undefined
  ) {
...
const buildMonitorDraft = (rawText: string): MonitorDraft | null => {
  ...
  const normalizedUrl = /^(?:https?:\/\/|wss?:\/\/)/i.test(candidate)
    ? candidate
    : `https://${candidate}`;
...
const looksLikeMonitorCreationRequest = (rawText: string): boolean => {
  ...
  const hasUrlCandidate = Boolean(
    structuredFields.url ?? readUrlCandidate(text),
  );
  if (!hasUrlCandidate) {
    return false;
  }
...
function AssistantChatbot({
  ...
  const startOrStopVoiceInput = (): void => {
    ...
    recognition.onstart = () => {
      setIsListening(true);
      setComposerNotice("Voice dictation is active. Speak now.");
    };
    ...
    recognition.onerror = (event) => {
      ...
      setComposerNotice(
        event.error
          ? `Voice dictation unavailable: ${event.error}.`
          : "Voice dictation encountered an error.",
      );
    };
    recognition.onend = () => {
      setIsListening(false);
      speechRecognitionRef.current = null;
      if (composerNotice?.startsWith("Voice dictation is active")) {
        setComposerNotice(null);
      }
    };
    ...
    try {
      ...
      speechRecognitionRef.current = null;
      setIsListening(false);
      setComposerNotice("Unable to start voice dictation.");
    }
  };
  ...
  const retryLastMessage = (): void => {
    if (!lastUserContent || isSending) {
      return;
    }
    // Remove the last error message and resend the user's message
    setMessages((current) => {
      const withoutLastError = current.slice(0, -1);
      return withoutLastError;
    });
    // Resend the last user message after a small delay to ensure UI updates
    window.setTimeout(() => {
      submitMessage(lastUserContent);
    }, 100);
  };
  ...
  const submitMessage = async (rawMessage: string): Promise<void> => {
    ...
    const outgoingContent = [content, attachmentSummary]
      .filter((part) => part !== "")
      .join("\n\n");
    ...
    const monitorDraft = looksLikeMonitorCreationRequest(content)
      ? buildMonitorDraft(content)
      : null;
    ...
    try {
      ...
      const fallbackReply =
        "I could not get a text response right now. Please try again.";
      ...
      if (!response.body) {
        ...
        const finalText =
          ...
          (parsedText.content.trim() || fallbackReply);
      ...
      {isOpen ? (
        ...
        >
          <header className="assistant-header">
            <button
              type="button"
              className="assistant-header-back"
              onClick={closeChat}
              aria-label="Back"
            >
              <ChevronLeft size={16} />
            ...
            <div className="assistant-header-actions" ref={headerActionsRef}>
              <div className="assistant-header-menu-anchor">
                ...
                {isMoreMenuOpen ? (
                  <div
                    className="assistant-header-menu"
                    role="menu"
                    aria-label="Options du chatbot"
                  >
                    <button
                      type="button"
                    ...
                    >
                      <span
                        className="assistant-header-menu-item-icon"
                        aria-hidden="true"
                      >
                        {isPanelExpanded ? (
                          <Minimize2 size={14} />
          ...
          <div className="assistant-body" ref={bodyRef}>
            {messages.map((message, index) => (
              ...
              >
                <div
                  className={`assistant-bubble ${message.role} ${message.isError ? "error" : ""}`}
                >
                  <p>{message.content}</p>
                  {message.isError && lastUserContent ? (
                    <button
                      type="button"
                ...
                {index === 0 && message.role === "assistant" ? (
                  <p className="assistant-message-meta">
                    Uptime Warden · AI Agent · Just now
                  </p>
                ) : null}
              </article>
            ))}
          ...
          <footer className="assistant-footer">
            <div className="assistant-input-shell" ref={composerShellRef}>
              ...
              {draftAttachments.length > 0 ? (
                <div
                  className="assistant-attachment-list"
                ...
                >
                  {draftAttachments.map((attachment) => (
                    ...
                    >
                      <span
                        className="assistant-attachment-chip-icon"
                        aria-hidden="true"
                      >
                      ...
                      <span className="assistant-attachment-chip-text">
                        <span className="assistant-attachment-chip-name">
                          {attachment.file.name}
                        </span>
                        <span className="assistant-attachment-chip-size">
              ...
              {isEmojiPickerOpen ? (
                <div
                  className="assistant-emoji-popover"
                  ref={emojiPanelRef}
                  role="dialog"
              ...
              <div className="assistant-input-toolbar">
                ...
                >
                  {isSending ? (
                    <LoaderCircle
                      size={16}
                      className="assistant-send-spinning"
                    />
                  ) : (
                    <Send size={16} />
                  )}
