...
const COMMON_LOCAL_BACKEND_TARGETS = [
...
const LOCAL_DIRECT_BACKEND_FALLBACKS = Array.from(
  new Set(
    COMMON_LOCAL_BACKEND_TARGETS.filter((target) => target.trim() !== ""),
  ),
);
...
export interface BackendMonitorPredictionDriver {
  label: string;
  value: string;
  impact: PredictionRiskLevel;
}

export interface BackendMonitorPredictionEntry {
  type: PredictionType;
  title: string;
  ...
  drivers: BackendMonitorPredictionDriver[];
}

export interface BackendMonitorPrediction {
  generatedAt: string;
  overallRiskLevel: PredictionRiskLevel;
  overallRiskScore: number;
  summary: string;
  predictions: BackendMonitorPredictionEntry[];
  signals: {
    sampleSize: number;
...
export const resolveAvatarUrl = (
  ...
  try {
    const baseUrl = isHttpUrl(API_BASE_URL)
      ? API_BASE_URL
...
export const login = (
  ...
  password: string,
  rememberMe = true,
): Promise<{ message: string; requiresOtp?: boolean; email?: string }> =>
  request<{ message: string; requiresOtp?: boolean; email?: string }>(
    "/auth/login",
    {
      method: "POST",
...
export const fetchMonitorPrediction = (
  monitorId: string,
  token?: string,
): Promise<{ prediction: BackendMonitorPrediction }> =>
  request<{ prediction: BackendMonitorPrediction }>(
    `/monitors/${monitorId}/prediction`,
    { token },
  );
