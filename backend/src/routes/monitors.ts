...
/**
 * GET /api/monitors/:id/prediction
 * Obtenir une prediction court terme pour un monitor
 */
router.get(
  "/:id/prediction",
  authenticate,
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      monitor.manuallyResumed = false; // Réinitialiser le flag
      await monitor.save();
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      monitor.pausedByMaintenance = false;
      monitor.manuallyResumed = true; // L'utilisateur override manuellement la maintenance
      await monitor.save();
...
/**
 * POST /api/monitors/:id/test-alert
 * Force une alerte DOWN pour tester les intégrations
 */
router.post(
  "/:id/test-alert",
  authenticate,
