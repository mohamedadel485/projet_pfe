...
class IntegrationService {
  ...
  private static readonly TELEGRAM_BOT_API_PATTERN = /^https?:\/\/api\.telegram\.org\/bot[^/]+\/sendMessage/i;
  ...
  private isTelegramBotApi(endpointUrl: string): boolean {
    return IntegrationService.TELEGRAM_BOT_API_PATTERN.test(endpointUrl);
  }
  ...
  private buildTelegramPayload(
    ...
    if (notification.monitor) {
      lines.push('', `Monitor: ${notification.monitor.name}`, `URL: ${notification.monitor.url}`);
      if (notification.monitor.currentStatus) {
        lines.push(`Status: ${notification.monitor.currentStatus.toUpperCase()}`);
      }
      if (typeof notification.monitor.responseTime === 'number') {
        lines.push(`Response time: ${notification.monitor.responseTime} ms`);
      }
    }

    return {
      chat_id: integration.customValue?.trim() || undefined,
      text: lines.join('\n'),
  ...
  private buildPayloadForIntegration(
    ...
    if (integration.type === 'telegram' || this.isTelegramBotApi(integration.endpointUrl)) {
      return this.buildTelegramPayload(integration, notification);
    }
  ...
  private async postToIntegration(
    ...
    try {
      console.log(`📤 Sending ${integration.type} notification to ${integration.endpointUrl}`);
      ...
      if (response.status >= 400) {
        ...
        console.error(
          `   Response body from ${integration.type}:`,
          responseBody
        );
        if (integration.type === 'telegram' && responseBody.includes('chat not found')) {
          console.error(
            '   Telegram target chat was not found. Check that customValue is a real channel/group username or numeric chat id, and that the bot was added to the channel/group with permission to post messages.'
          );
        }
        console.error(
      ...
      console.log(`✅ Notification sent successfully to ${integration.type} (${response.status})`);
      return true;
    } catch (error: any) {
      console.error(
  ...
  private async notifyUsersByEmail(
    payload: MonitorStatusChangePayload,
    recipientUserIds: string[],
  ): Promise<void> {
    if (payload.monitor.emailNotificationsEnabled === false) {
      console.log('📧 Notifications email desactivees pour ce monitor.');
      return;
    }
    ...
    try {
      ...
      if (recipients.length === 0) {
        console.log('📧 Aucun destinataire email actif pour cette alerte.');
        return;
      }

      console.log(
        `📧 Envoi alerte ${payload.result.status.toUpperCase()} par email a ${recipients.length} destinataire(s)`,
      );

      await Promise.all(
        recipients.map((email) =>
          emailService.sendMonitorAlert(
            email,
      ...
      console.error('❌ Erreur lors de l envoi des alertes email:', error);
    }
  }
  ...
  async notifyMonitorStatusChange(payload: MonitorStatusChangePayload): Promise<void> {
    ...
    console.log(`📢 Monitor ${payload.monitor.name} changed to ${payload.result.status}, notifying ${recipientUserIds.length} user(s)`);

    await this.notifyUsersByEmail(payload, recipientUserIds);
    ...
    console.log(`📬 Found ${integrations.length} integration(s) to notify for event: ${event}`);

    if (integrations.length === 0) {
      console.log('⚠️  No integrations configured for this event.');
      return;
    }
  ...
  async sendIntegrationTest(integration: IIntegration): Promise<void> {
    ...
    console.log(`🧪 Sending test notification for ${integration.type} integration ${integration._id}`);
